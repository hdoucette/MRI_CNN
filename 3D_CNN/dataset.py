...
if torch.cuda.is_available():
    for file in os.listdir(train_root):
        ...
        mri=mri['data']
        if i==0:
            tensor_x=torch.from_numpy(mri[0][0]).float().to(device)
            tensor_y=torch.from_numpy(mri[0][1]).float().to(device)
        else:
            tensor_x=torch.stack(tensor_x,torch.from_numpy(mri[0][0]).float()).to(device)
            tensor_y = torch.stack(tensor_y, torch.from_numpy(mri[0][1]).float()).to(device)
        i+=1
