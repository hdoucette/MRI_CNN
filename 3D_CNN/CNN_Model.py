...
class MRI_CNN(nn.Module):
    def __init__(self, num_classes=3):
        ...
        self.features = nn.Sequential(
            ...
            nn.MaxPool3d(kernel_size=(6,6,6),stride=2,padding=0),
            nn.Conv3d(16,32,kernel_size=(5,5,5),stride=2,padding=0),
            nn.BatchNorm3d(32),
            nn.LeakyReLU(inplace=True),
            nn.MaxPool3d(kernel_size=(5,5,5), stride=2, padding=0),
            nn.Conv3d(32, 64, kernel_size=(4, 4, 4), stride=2, padding=0),
            nn.BatchNorm3d(64),
            nn.LeakyReLU(inplace=True),
            nn.MaxPool3d(kernel_size=(4, 4, 4), stride=2, padding=0),
            nn.Dropout(.5))
